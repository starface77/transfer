...
class SharoukinDesktopApp:
    """Google Jules-inspired multi-panel desktop reference for Sharrowkin."""

    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Sharrowkin Developer Agent")
        ...
        self.colors = {
            ...
            "accent_pink": "#C85F2B",   # Warm orange
            "success_green": "#2E7D4F",    # Success green
            "muted_blue": "#3F6F7F",     # Muted blue
            "diff_violet": "#6652A7",   # Diff header violet
            "warm_amber": "#D69A2D",   # Warm amber
    ...
    def setup_ui(self) -> None:
        ...
        self.path_entry = tk.Entry(
            ...
            highlightcolor=self.colors["muted_blue"],
        )
        ...
        self.launch_btn = tk.Button(
            ...
            fg="#FFFFFF",
            activebackground=self.colors["diff_violet"],
        ...
        self.diff_area.tag_config("header", foreground=self.colors["diff_violet"], font=("Consolas", 9, "bold"))
        self.diff_area.tag_config("info", foreground=self.colors["muted_blue"])

        # Config tags for Log terminal coloring
        self.log_area.tag_config("info", foreground=self.colors["muted_blue"])
        self.log_area.tag_config("success", foreground=self.colors["success_green"])
        self.log_area.tag_config("warning", foreground=self.colors["warm_amber"])
        self.log_area.tag_config("error", foreground=self.colors["accent_pink"], font=("Consolas", 9, "bold"))
        self.log_area.tag_config("system", foreground=self.colors["diff_violet"], font=("Consolas", 9, "bold"))
    ...
    def update_step(self, step_idx: int, status: str) -> None:
        ...
        if status == "active":
            ...
            label.config(text=f"✅ {original_text}", fg=self.colors["success_green"], font=("Consolas", 9))
        else:
    ...
    def start_agent(self) -> None:
        ...
        self.status_indicator.config(text="RUNNING", fg=self.colors["accent_pink"])
    ...
    def agent_completed(self, success: bool) -> None:
        ...
        if success:
            self.status_indicator.config(text="STABLE", fg=self.colors["success_green"])
        else:
            self.status_indicator.config(text="ERROR", fg=self.colors["accent_pink"])
