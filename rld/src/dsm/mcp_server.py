...
class DsmMcpServer:
    ...
    def __init__(self, memory: DynamicSegmentedMemory, rld: RecursiveLatentDNA | None = None):
        self.memory = memory
        self.rld = rld or RecursiveLatentDNA(memory.storage.path.with_name("rld_genes.json"))
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "rld_observe":
            ...
            self.rld.save()
            return text_result({"trajectory_id": trajectory.id, **self.rld.stats()})
        if name == "rld_activate":
            context = self.rld.active_context(
                str(arguments["query"]),
